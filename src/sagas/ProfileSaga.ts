...
} from '../interface/profile/IProfileResponse';
import {IUser, IUserResponse} from '../interface/user/IUser';

export function* updateProfileSaga(action: PayloadAction<Partial<IProfile>>) {
  try {
    const response: IProfileUpdateResponse = yield call(
...
export function* updateUserSubscriptionSaga(action: PayloadAction<IUser>) {
  try {
    const response: IUserResponse = yield call(
      ProfileService.updateUserSubscription,
      action.payload,
    );
    console.log(
      'updateUserSubscriptionSaga--response--',
